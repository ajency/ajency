	...
	public static function optionsframework_machine($options) {
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				//space
				case 'space': 
					$output .= '<div style="clear:both; display: block; height:50px; width:100%"></div>';
				break;
				...
				//typography option	
				case 'google_typography':
				
					$typography_stored = isset($smof_data[$value['id']]) ? $smof_data[$value['id']] : $value['std'];
					
	
							global $webfonts;
							$fonts = json_decode($webfonts,true);
							$items = $fonts['items'];	
							$faces = array();
							$str = '';
							foreach($items as $item) {
								$str = str_replace(' ', '+', $item['family']) . ':';
								$rep = '';
							...
							array_unshift_assoc($faces, 'none', 'Select a font');		*/				
					
					/* Google Font Face */
					if(isset($typography_stored['face'])) {						
												
						$output .= '<div class="select_wrapper typography-google-face" original-title="Google Font family">';
						$output .= '<select class="of-typography of-typography-google-face select" name="'.$value['id'].'[face]" id="'. $value['id'].'_face">';
				...
				//drag & drop client manager
				case 'client':
					$output .= '<div class="slider"><ul id="'.$value['id'].'">';
					$clients = $smof_data[$value['id']];
					$count = count($clients);
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
						$output .= Options_Machine::optionsframework_client_function($value['id'],$value['std'],$oldorder,$order);
					} else {
						$i = 0;
						foreach ($clients as $slide) {
							$oldorder = $slide['order'];
							$i++;
							$order = $i;
							$output .= Options_Machine::optionsframework_client_function($value['id'],$value['std'],$oldorder,$order);
				...
				case 'client':
					...
					$clients = $data[$value['id']];
					$counting = count($clients);
					if ($counting < 2) {
						$orderold = 1;
						$order = 1;
						$output .= Options_Machine::optionsframework_clients_function($value['id'],$value['std'],$orderold,$order,$int);
					} else {
						$i = 0;
						foreach ($clients as $client_item) {
							$orderold = $client_item['order'];
							$i++;
							$order = $i;
							$output .= Options_Machine::optionsframework_clients_function($value['id'],$value['std'],$orderold,$order,$int);
						}
				...
				case 'backup':
					...
					$output .= '<p><strong>'. __('Last Backup : ', 'delicious').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
