...
<?php 
	...
	$individual_class="";
	$individual_lightbox=false;
	if (isset($data['alchemy_posts_shape']) && $data['alchemy_posts_shape']=="squares") {
		$posts_shape="squares";
	}
	else {
		$posts_shape="rectangles";
	}
	//$individual_lightbox=true;
	//$individual_class=' class="individuals"';
...
<div id="main_block" class="clearfix fff_folio page-<?php echo get_the_ID(); ?>">
    <?php
						...
						<div id="post-<?php the_ID(); ?>" class="<?php echo $featured_class; ?>boxed_shadow portfolio_entry_li <?php echo $skills_yo; ?> p_all" data-id="id-<?php echo $ins; ?>" data-color="<?php echo $featured_color; ?>">
							...
							<div class="grid_image_wrapper boxed_shadow">
								<?php 
									...
									{
										?>
										<div class="prk_magnificent body_bk_color<?php echo $vid_cl;?>" data-mfp-src="<?php echo esc_attr($magnific_image[0]); ?>">
									...
									{
										?>
										<div class="prk_magnificent_li prk_magnificent_li_outer  body_bk_color" data-mfp-src="<?php echo esc_attr($magnific_image[0]); ?>">
											<div class="navicon-expand-2"></div>
										</div>
										<?php
									}
									if ($make_lbox!="yes" && $mini_sliders=="yes" && $data['skip_featured']==1)
									{
                                        ...
                                        {
                                       		?>
											<div class="prk_magnificent_li prk_magnificent_li_outer body_bk_color" data-mfp-src="<?php echo esc_attr($data['image_2']); ?>">
												<div class="navicon-expand-2"></div>
											</div>
											<?php
                                        }
                                        ...
                                        {
                                        	echo '<div class="prk_magnificent_li prk_magnificent_li_outer body_bk_color mfp-iframe" data-mfp-src="'.esc_attr(get_iframe_src($data['image_2'])).'">';
                                            echo '<div class="navicon-expand-2"></div>';
                                            echo '</div>';
                                        }
									}
								?>
                            	<a href="<?php echo $href_val; ?>" class="<?php if ($make_lbox=="yes" && $mini_sliders=="no"){echo 'magna_a'; echo $vid_cl;} else { if ($ajax_single_layout!=""){echo "prk_trigger_ajax";} else { if ($data['skip_to_external']==0) {echo "fade_anchor";}}} ?>" data-ajax_id="<?php the_ID(); ?>" data-ajax_order="<?php echo $ins; ?>" data-color="<?php echo $featured_color; ?>">
								...
								?>
                                    ...
                                    if (has_post_thumbnail( $post->ID ) )
                                    {
										...
										else
										{
											...
											if ($forced_w<round(768/2))
											{	
                                				$forced_w=round(768/2);//BECAUSE OF RESPONSIVE 2 COLS
                                			}
										}
										//DOUBLE SIZE THUMB?
										if ($data['featured_thumb']==0 || $posts_shape=="squares") {
											if ($posts_shape=="squares") {
												$forced_h=$forced_w;
											}
											else {
											...
											<div id="tiny_slider-<?php echo $post->ID;?>" <?php if ($mini_sliders=="yes"){echo 'class="tiny_slider per_init"';} ?>>
												<ul class="slides">
													<li>
														<img src="<?php echo $vt_image['url']; ?>" width="<?php echo $vt_image['width']; ?>" height="<?php echo $vt_image['height']; ?>" id="home_fader-<?php the_ID(); ?>" class="custom-img grid_image" data-featured="no" alt="<?php echo prk_get_img_alt($image[0]); ?>" />
											<?php
										}
										else
										{
											...
											<ul class="slides">
												<li>
													<img src="<?php echo $vt_image['url']; ?>" width="<?php echo $vt_image['width']; ?>" height="<?php echo $vt_image['height']; ?>" id="home_fader-<?php the_ID(); ?>" class="custom-img grid_image" data-featured="yes" alt="<?php echo prk_get_img_alt($image[0]); ?>" />
	                                        ...
	                                        for ($count=$jumper;$count<21;$count++)
	                                        {
	                                            if (isset($data['image_'.$count]) && $data['image_'.$count]!="")
	                                            {
                                                    if (prk_external_content($data['image_'.$count])=="other")
                                                    {
                                                    	echo "</li>";
                                                    	echo "<li>";
                                                    	$image_alt=$data['image_'.$count];
                                                   		$vt_image = vt_resize( '', $data['image_'.$count] , $forced_w, $forced_h, true , $retina_flag );
                                                    	echo '<img src="'.$vt_image['url'].'" width="'. $vt_image['width'] .'" height="'. $vt_image['height'] .'" class="grid_image" alt="'.prk_get_img_alt($image_alt).'" />';
	                                                	...
	                                                	{
	                                                    	echo '<div class="prk_magnificent_li" data-mfp-src="'.esc_attr($data['image_'.$count]).'"></div>';
	                                                    }
	                                                }
	                                                else
	                                                {
	                                                	echo '<div class="prk_magnificent_li body_bk_color mfp-iframe" data-mfp-src="'.esc_attr(get_iframe_src($data['image_'.$count])).'">';
                                                        echo '<div class="navicon-expand-2"></div>';
                                                        echo '</div>';
	                                                }
	                                            }
                                ...
                                <img src="<?php echo $vt_image['url']; ?>" class="hide_now" alt="<?php echo prk_get_img_alt($image[0]); ?>" />
                                </a>
                            </div>
							</div>
						<?php $ins++; ?>
...
{
	echo '<div id="centered_block" class="prk_inner_block columns centered"><div id="main_block">';
		while (have_posts()) : the_post();
	    	the_content();   
	    endwhile;
	if (INJECT_STYLE) {
