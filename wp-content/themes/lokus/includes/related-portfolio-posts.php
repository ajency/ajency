  		...
  		{ 
     		...
     		{	
	 			...
	 			{	
	 				...
	 				{
	 					$invisible_image = $image = get_post_meta(get_the_ID(), 'Project_logo', True);
	 				}
	 				else
	 				{
     			...
     			$img_src = get_post_meta(get_the_ID(), 'Project_logo', True);
				
     			$output .= "<div class='$single_class relThumb relThumb".$count."'>\n";
	 			$output .= "<a href='".get_permalink()."' class='relThumWrap noLightbox'>\n";
     			$output .= "<span class='related_image_wrap'>";
	 			$output .= "<img src='".$img_src."' class='logo-fade' />";
	 			$output .= "</span>\n";
