		...
		return this.each(function()
		{
			...
			var methods = 
			{
				avia_add_google_font: function()
				{
					...
					{
						insert = '<link rel="stylesheet" type="text/css" href="http://fonts.googleapis.com/css?family='+cssValue+'" /> <style type="text/css">.webfont_'+this.id+'{font-family:'+value+';}</style>';
					}
		...
		var win			= $(window),
			...
			methods		= {
				...
				closeListener: function(container, close_element)
				{
					...
					{
						if(close_element.get(0) != event.target && close_element.find(event.target).length == 0)
						{
