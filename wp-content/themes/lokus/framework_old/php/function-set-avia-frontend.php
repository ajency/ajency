...
if(!function_exists('avia_image_by_id'))
{
	...
	{	
		...
		if(is_object($attachment))
		{
			...
			return "<img src='".$image_src[0]."'  alt='".$image_description."' ".$data."/>";
		}
	}
}
...
if(!function_exists('avia_get_link'))
{
	...
	*/
	...
	{	
		...
		{
			return "<a href='".$url."' target='_blank'>".$inside."</a>";
		}
	}
}
	...
	{	
		...
		elseif (is_search()) 
		{
			...
			if(!empty($wp_query->found_posts))
			{
				...
				{
					$output =  $wp_query->found_posts ." ". __('search results for: ','avia_framework').$_GET['s']; 
				}
				else
				{
					$output =  $wp_query->found_posts ." ". __('search result for: ','avia_framework').$_GET['s']; 
				}
			}
			else
			{
