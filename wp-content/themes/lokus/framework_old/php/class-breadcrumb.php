...
if(!class_exists('avia_breadcrumb'))
{
	...
	function avia_breadcrumb($options = ""){
		...
		if(is_404()){ return _e("404 - Page not Found",'avia_framework'); }
		
		if(is_search()){ return _e("Search",'avia_framework');}	
	...
	elseif ( is_singular() ) {
		...
		*/
		...
		if($post_type == 'portfolio')
		{
			if(isset($_GET['lcat']))
			{
				$term		= get_term_by('id', $_GET['lcat'] , 'portfolio_entries' );
				
				if($term)
					$term_link  = get_term_link($term->slug, 'portfolio_entries' );
			
				if($term_link && !is_wp_error($term_link))
					$trail[] = '<a href="'.$term_link.'">'.$term->name.'</a>';
				else
					$trail[] = '<a href="'.home_url('/work/').'">Work</a>';
			}
			else
			{
				...
				foreach ($parents as $parent_item)
				{
					if($parent_item) $post_categories = $post_categories .", ".$parent_item;
					 
				}
				$trail[] = rtrim(ltrim($post_categories,', '),', ');
			}
