...
(function( $ ) {
    ...
    redux.field_objects.typography_mod = redux.field_objects.typography_mod || {};
    ...
    $( document ).ready(
        function() {
            //redux.field_objects.typography_mod.init();
        }
    );

    redux.field_objects.typography_mod.init = function( selector ) {

        if ( !selector ) {
            selector = $( document ).find( '.redux-container-typography_mod' );
        }
                    ...
                    function() {
                        ...
                        $( this ).find( '.redux-typography-container' ).each(
                            function() {
                                ...
                                redux.field_objects.typography_mod.select( family );
                        ...
                        $( this ).find( '.redux-typography' ).on(
                            'change', function() {
                                redux.field_objects.typography_mod.select( $( this ) ); //.parents('.redux-container-typography_mod:first'));
                            }
                        );
                        ...
                        $( this ).find( '.redux-typography-color' ).wpColorPicker(
                            {
                                change: function( event, ui ) {
                                    ...
                                    redux.field_objects.typography_mod.select( $( this ) );
                                }
                            }
                        );
                        ...
                        $( this ).find( ".redux-typography-family" ).select2(
                            ...
                            "select2-opening", function( e ) {
                                ...
                                // Get field ID
                                var thisID = $( this ).parents( '.redux-container-typography_mod:first' ).attr( 'data-id' );
    ...
    // Return font size
    redux.field_objects.typography_mod.size = function( obj ) {
        var size = 0,
            key;
    ...
    redux.field_objects.typography_mod.makeBool = function( val ) {
    ...
    redux.field_objects.typography_mod.contrastColour = function( hexcolour ) {
    ...
    redux.field_objects.typography_mod.select = function(selector) {

        // Main id for selected field
        var mainID          = $(selector).parents('.redux-container-typography_mod:first').attr('data-id');
        ...
        var google;
        if (isSelecting === true) {
            google = redux.field_objects.typography_mod.makeBool(selVals.object['data-google']);
            $('#' + mainID + ' .redux-typography-google-font').val(google);
        } else {
            google = redux.field_objects.typography_mod.makeBool($('#' + mainID + ' .redux-typography-google-font').val()); // Check if font is a google font
        }
        ...
        if ($(selector).hasClass('redux-typography-family')) {
            ...
            if (google === true) {
                ...
                $.each(details.variants, function(index, variant) {
                    if (variant.id === style || redux.field_objects.typography_mod.size(details.variants) === 1) {
                ...
                $.each(details.subsets, function(index, subset) {
                    if (subset.id === script || redux.field_objects.typography_mod.size(details.subsets) === 1) {
                        selected = ' selected="selected"';
                        script = subset.id;
                        $('#' + mainID + ' input.typography-subsets').val(script);                        
                    } else {
        ...
        if ($('#' + mainID).hasClass('typography-initialized')) {
            ...
            if (color) {
                ...
                $('#' + mainID + ' .typography-preview').css('background-color', redux.field_objects.typography_mod.contrastColour(color));
            }
