...
jQuery(document).ready(function () {
	...
	jQuery('.social-selector-sel').on('change', function() {

		var _val = jQuery(this).val();

		jQuery(this).parent().siblings('.social-icon-receiver').find('input').val( _val );

	});

	jQuery('.social-icon-receiver input').each( function(e, i) {

		var _val = jQuery(this).val();

		jQuery(this).parent().siblings('.social-icon-changer').find('select').val( _val );

	});

    jQuery('.redux-social-select-remove').live('click', function () {
        ...
        var slideCount = jQuery(this).parents('.redux-container-social_select:first').find('.redux-slides-accordion-group').length;
        ...
        if (slideCount > 1) {
            ...
            jQuery(this).parents('.redux-container-social_select:first').find('.redux-slides-accordion-group:last').find('.redux-slides-header').text("New Slide");            
        }
    });

    jQuery('.redux-social-select-add').click(function () {
