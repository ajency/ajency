			...
			foreach ($sidebars['sidebars'] as $sidebar) {
				if( isset($sidebar) && !empty($sidebar) ){
					...
					register_sidebar(
						array(
							'name'          => $sidebar['name'],
							'id'            => 'dt_sbar_' . $sidebar['id'],
							'description'   => '',
							'before_widget' => '<div id="%1$s" class="widget dt_sbar_' . $sidebar['id'] .' %2$s">',
							'after_widget'  => '</div>',
							'before_title'  => '<h3>',
							'after_title'   => '</h3>'
						)
					...
					foreach ($sidebars['sidebars'] as $sidebar) {
						if( isset($sidebar) && !empty($sidebar) ){
							...
							echo '<div class="smk_sbg_one_sidebar">
								...
								<span class="smk_sbg_code"><code>dt_sidebar("dt_sbar_' . $sidebar['id'] . '");</code></span>
								<span class="smk_sbg_code"><code>[dt_sidebar id="' . $sidebar['id'] . '"]</code></span>
...
----------------------------------------------------------------------
*/
// [dt_sidebar id="X"] //X is the sidebar number
function dt_sidebar_shortcode( $atts ) {
	
	extract( shortcode_atts( array(
		'id' => null,
	), $atts ) );

	dt_sidebar('dt_sbar_' . $id);

}
add_shortcode( 'dt_sidebar', 'dt_sidebar_shortcode' );