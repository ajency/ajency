...
class Decent_Comments_Widget extends WP_Widget {
	...
	function Decent_Comments_Widget() {
		parent::WP_Widget( false, $name = 'Samba: Better Comments' );
	}
	...
	function form( $instance ) {
		...
		echo "<p>";
		echo '<label for="' .$this->get_field_id( 'title' ) . '">' . __( 'Title', 'samba_lang' ) . '</label>'; 
		...
		echo "<p>";
		echo '<label class="title" title="' . __( "The number of comments to show.", 'samba_lang' ) .'" for="' .$this->get_field_id( 'number' ) . '">' . __( 'Number of comments', 'samba_lang' ) . '</label>'; 
		...
		echo '<p>';
		echo '<label class="title" title="' . __( "Sorting criteria.", 'samba_lang' ) .'" for="' .$this->get_field_id( 'orderby' ) . '">' . __( 'Order by ...', 'samba_lang' ) . '</label>';
		...
		echo '<p>';
		echo '<label class="title" title="' . __( "Sort order.", 'samba_lang' ) .'" for="' .$this->get_field_id( 'order' ) . '">' . __( 'Sort order', 'samba_lang' ) . '</label>';
		...
		echo "<p>";
		echo '<label class="title" title="' . __( "Leave empty to show comments for all posts. To show comments for a specific post only, indicate either part of the title or the post ID. To show posts for the current post, indicate: [current]", 'samba_lang' ) . '" for="' .$this->get_field_id( 'post_id' ) . '">' . __( 'Post ID', 'samba_lang' ) . '</label>'; 
		echo '<input class="widefat" id="' . $this->get_field_id( 'post_id' ) . '" name="' . $this->get_field_name( 'post_id' ) . '" type="text" value="' . esc_attr( $post_id ) . '" />';
		echo '<br/>';
		echo '<span class="description">' . __( "Title, empty, post ID or [current]", 'samba_lang' ) . '</span>';
		if ( !empty( $post_id ) && ( $post_title = get_the_title( $post_id ) ) ) {
			echo '<br/>';
			echo '<span class="description"> ' . sprintf( __("Selected post: <em>%s</em>", 'samba_lang' ) , $post_title ) . '</span>';
		}
		echo '</p>';
		...
		echo '<label class="title" title="' . __( "If checked, shows an excerpt of the comment. Otherwise the full text of the comment is displayed.", 'samba_lang' ) .'" for="' . $this->get_field_id( 'excerpt' ) . '">' . __( 'Show comment excerpt', 'samba_lang' ) . '</label>';
		echo '</p>';
		
		// max_excerpt_words
		$max_excerpt_words = isset( $instance['max_excerpt_words'] ) ? intval( $instance['max_excerpt_words'] ) : '';
		echo "<p>";
		echo '<label class="title" title="' . __( "The maximum number of words shown in excerpts.", 'samba_lang' ) .'" for="' .$this->get_field_id( 'max_excerpt_words' ) . '">' . __( 'Number of words in excerpts', 'samba_lang' ) . '</label>'; 
		...
		echo "<p>";
		echo '<label class="title" title="' . __( "The ellipsis is shown after the excerpt when there is more content.", 'samba_lang' ) . '" for="' .$this->get_field_id( 'ellipsis' ) . '">' . __( 'Ellipsis', 'samba_lang' ) . '</label>'; 
		...
		echo '<label class="title" title="' . __( "Whether to show the author of each comment.", 'samba_lang' ) .'" for="' . $this->get_field_id( 'show_author' ) . '">' . __( 'Show author', 'samba_lang' ) . '</label>';
		echo '</p>';
		...
		echo '<label class="title" title="' . __( "Show the avatar of the author.", 'samba_lang' ) .'" for="' . $this->get_field_id( 'show_avatar' ) . '">' . __( 'Show avatar', 'samba_lang' ) . '</label>';
		echo '</p>';

		// avatar size
		$avatar_size = isset( $instance['avatar_size'] ) ? intval( $instance['avatar_size'] ) : '';
		echo "<p>";
		echo '<label class="title" title="' . __( "The size of the avatar in pixels.", 'samba_lang' ) .'" for="' .$this->get_field_id( 'avatar_size' ) . '">' . __( 'Avatar size', 'samba_lang' ) . '</label>'; 
		...
		echo '<label class="title" title="' . __( "Show a link to the post that the comment applies to.", 'samba_lang' ) .'" for="' . $this->get_field_id( 'show_link' ) . '">' . __( 'Show link to post', 'samba_lang' ) . '</label>';
		echo '</p>';
		...
		echo '<label class="title" title="' . __( "Show an excerpt of the comment or the full comment.", 'samba_lang' ) .'" for="' . $this->get_field_id( 'show_comment' ) . '">' . __( 'Show the comment', 'samba_lang' ) . '</label>';
		echo '</p>';
		
		// taxonomy & terms
		$taxonomy = isset( $instance['taxonomy'] ) ? $instance['taxonomy'] : '';
		echo "<p>";
		echo '<label class="title" title="' . __( "A taxonomy, e.g. category or post_tag", 'samba_lang' ) .'" for="' .$this->get_field_id( 'taxonomy' ) . '">' . __( 'Taxonomy', 'samba_lang' ) . '</label>';
		...
		echo '<br/>';
		echo '<span class="description">' . __( "Indicate <strong>category</strong> if you would like to show comments on posts in certain categories. Give the desired categories' slugs in <strong>Terms</strong>. For tags use <strong>post_tag</strong> and give the tags' slugs in <strong>Terms</strong>.", 'samba_lang' ) . '</span>';
		echo '</p>';
		...
		echo "<p>";
		echo '<label class="title" title="' . __( "If a taxonomy is given , indicate terms in that taxonomy separated by comma to show comments for all posts related to these terms. To show comments on posts related to the same terms as the current post, indicate: {current}. If a taxonomy is given and terms is empty, no comments will be shown.", 'samba_lang' ) . '" for="' .$this->get_field_id( 'terms' ) . '">' . __( 'Terms', 'samba_lang' ) . '</label>';
		echo '<input class="widefat" id="' . $this->get_field_id( 'terms' ) . '" name="' . $this->get_field_name( 'terms' ) . '" type="text" value="' . esc_attr( $terms ) . '" />';
		echo '<br/>';
		echo '<span class="description">' . __( "Terms or {current}. A <strong>Taxonomy</strong> must be given.", 'samba_lang' ) . '</span>';
		echo '</p>';
		echo "<p>";
