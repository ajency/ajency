...
class Decent_Comments_Renderer {
	...
	static function init() {
		self::$orderby_options = array(
			'comment_author_email' => __( 'Author Email', 'samba_lang' ),
			'comment_author_url'   => __( 'Author URL', 'samba_lang' ),
			'comment_content'      => __( 'Content', 'samba_lang' ),
			'comment_date_gmt'     => __( 'Date', 'samba_lang' ),
			'comment_karma'        => __( 'Karma', 'samba_lang' ),
			'comment_post_ID'      => __( 'Post', 'samba_lang' )
		);
		self::$order_options = array(
			'ASC'  => __( 'Ascending', 'samba_lang' ),
			'DESC' => __( 'Descending', 'samba_lang' )
		);
	}
	...
	static function get_comments( $options = array() ) {
		...
		if ( !empty( $comments ) ) {
			...
			$output .= '<div class="comments_slider decent-comments">';
			...
			foreach ( $comments as $comment) {
				...
				if ( $show_comment ) {
					$output .= '<span class="prk_speech comment-' . ( $excerpt ? "excerpt" : "body" ) . '">';
					$output .= '<div class="prk_inner_tip"></div><div class="tip_top_hide"></div>';
				...
				if ( $show_author ) {
					$output .= '<span class="comment-author zero_color fade_anchor">';
					$output .= get_comment_author_link( $comment->comment_ID );
					$output .= '</span>'; // .comment-author
				}
				
				if ( $show_link ) {				
					$output .= '<span class="comment-link zero_color fade_anchor">';
					$output .= sprintf(
						_x( ' on %s', 'comment-link', 'samba_lang' ),
