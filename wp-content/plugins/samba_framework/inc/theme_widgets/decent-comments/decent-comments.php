 ...
 */
define( 'DC_PLUGIN_DOMAIN', 'samba_lang' );
...
function DC_admin_menu() {
	//if ( function_exists('add_submenu_page') ) {
		//add_submenu_page( 'plugins.php', __( 'Pirenko: Better Comments Options', 'samba_lang' ), __( 'Pirenko: Better Comments', 'samba_lang' ), 'manage_options', 'decent-comments-options', 'DC_options');
	//}
}
...
function DC_options() {
	...
	if ( !current_user_can( "manage_options" ) ) {
		wp_die( __( 'Access denied.', 'samba_lang' ) );
	}
	
	echo
		'<div>' .
			'<h2>' .
				__( 'Pirenko: Better Comments Options', 'samba_lang' ) .
			'</h2>' .
		'</div>';
	...
	echo
		'<form action="" name="options" method="post">' .		
			'<div>' .
				'<h3>' . __( 'Settings', 'samba_lang' ) . '</h3>' .
				'<p>' .
					'<input name="delete-data" type="checkbox" ' . ( $delete_data ? 'checked="checked"' : '' ) . '/>' .
					'<label for="delete-data">' . __( 'Delete settings when the plugin is deactivated', 'samba_lang' ) . '</label>' .
				'</p>' .
				'<p>' .
					wp_nonce_field( plugin_basename( __FILE__ ), DC_OPTIONS_NONCE, true, false ) .
					'<input type="submit" name="submit" value="' . __( 'Save', 'samba_lang' ) . '"/>' .
				'</p>' .
			'</div>' .
		'</form>';
}
